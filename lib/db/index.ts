...
function acquireBootstrapLock(lockPath: string, timeoutMs = 20000) {
  ...
  while (true) {
    try {
      ...
      if (Date.now() - start >= timeoutMs) {
        throw new Error(`Timed out waiting for database bootstrap lock at ${lockPath}`);
      }
...
function releaseBootstrapLock(lockHandle: number, lockPath: string) {
  try {
...
function bootstrapDatabase() {
  ...
  const lockHandle = acquireBootstrapLock(lockPath);
  ...
  try {
    ...
    try {
      sqlite.exec(
        "CREATE VIRTUAL TABLE IF NOT EXISTS notes_search USING fts5(note_id UNINDEXED, title, body, manual_tags, derived_tags, anchor_label, opening, leak_label)"
      );
    } catch {}
    ...
    releaseBootstrapLock(lockHandle, lockPath);
  }
}
